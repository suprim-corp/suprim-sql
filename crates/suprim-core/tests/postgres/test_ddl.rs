...
async fn truncate_table() {
    ...
    driver.execute("CREATE TABLE IF NOT EXISTS ddl_truncate (id SERIAL PRIMARY KEY)").await.unwrap();
    driver.execute("INSERT INTO ddl_truncate DEFAULT VALUES").await.unwrap();
    driver.execute("INSERT INTO ddl_truncate DEFAULT VALUES").await.unwrap();
    driver.execute("INSERT INTO ddl_truncate DEFAULT VALUES").await.unwrap();

    driver.truncate_table("public", "ddl_truncate").await.unwrap();
...
async fn rename_table() {
    ...
    driver.execute("CREATE TABLE ddl_rename_src (id SERIAL PRIMARY KEY)").await.unwrap();

    driver.rename_table("public", "ddl_rename_src", "ddl_rename_dst").await.unwrap();
...
async fn drop_table() {
    let driver = helpers::connected_driver("testdb").await;
    driver.execute("CREATE TABLE IF NOT EXISTS ddl_drop (id SERIAL PRIMARY KEY)").await.unwrap();

    driver.drop_table("public", "ddl_drop").await.unwrap();
...
async fn create_and_drop_view() {
    ...
    driver.drop_view("public", "ddl_test_view").await.unwrap();
...
#[tokio::test]
async fn create_schema_in_database() {
    let driver = helpers::connected_driver("testdb").await;
    let _ = driver.execute("DROP SCHEMA IF EXISTS ddl_schema_test CASCADE").await;

    driver.create_schema("testdb", "ddl_schema_test").await.unwrap();

    let schemas = driver.list_schemas("testdb").await.unwrap();
    assert!(schemas.contains(&"ddl_schema_test".to_string()));

    driver.execute("DROP SCHEMA ddl_schema_test CASCADE").await.unwrap();
}
