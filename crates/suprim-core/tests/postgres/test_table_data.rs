...
async fn pagination_count_and_offset() {
    ...
    let page0 = driver
        .table_data(Some("testdb"), Some("public"), "orders", 0, 2, None, None)
        .await
        .unwrap();
    ...
    let page1 = driver
        .table_data(Some("testdb"), Some("public"), "orders", 1, 2, None, None)
        .await
        .unwrap();
    assert_eq!(page1.rows.len(), 2);

    let page2 = driver
        .table_data(Some("testdb"), Some("public"), "orders", 2, 2, None, None)
        .await
        .unwrap();
...
async fn where_clause_filters_rows() {
    ...
    let result = driver
        .table_data(
            Some("testdb"), Some("public"), "orders", 0, 50,
...
async fn where_and_order_combined() {
    ...
    let result = driver
        .table_data(
            Some("testdb"), Some("public"), "orders", 0, 50,
            Some("total > 100"), Some("total ASC"),
        )
        .await
        .unwrap();

    assert_eq!(result.rows.len(), 3, "3 orders have total > 100 (150, 200, 320)");
...
async fn no_database_param_uses_connection_default() {
    let driver = helpers::connected_driver("testdb").await;
    let result = driver
        .table_data(None, Some("public"), "orders", 0, 50, None, None)
        .await
        .unwrap();
    assert_eq!(result.rows.len(), 5);
}
...
async fn page_beyond_data_returns_empty() {
    let driver = helpers::connected_driver("testdb").await;
    let result = driver
        .table_data(Some("testdb"), Some("public"), "orders", 100, 50, None, None)
        .await
        .unwrap();
...
async fn nullable_metadata_correct() {
    let driver = helpers::connected_driver("testdb").await;
    let result = driver
        .table_data(Some("testdb"), Some("public"), "users", 0, 1, None, None)
        .await
        .unwrap();
...
#[tokio::test]
async fn no_schema_param_defaults_to_public() {
    let driver = helpers::connected_driver("testdb").await;
    // schema = None should default to "public" in PG
    let result = driver
